...
urls =
  development:
    app: "//127.0.0.1:3333"
    api: "//alegna-api.nerevu.com/v1" #localhost can't connect to quickbooks
...
compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|ipad|iris|kindle|Android|Silk|
lge|maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|
plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|
windows (ce|phone)|xda|xiino|1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|
abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|
...
kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|
m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|
...
pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|
60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|
...
mobileDevice = (/"#{mobileTerms}"/i).test ua?.toLowerCase()[...4]
...
mobile = debug.mobile or mobileDevice
...
module.exports =
  storage:
    local: dataSource is 'local'
    remote: dataSource is 'remote'
  dev: environment is 'development'
  prod: environment in ['staging', 'production']
  verbose: verbose
  ...
  ua: ua
  vpw: window?.innerWidth or document?.documentElement?.clientWidth or 0
  vph: window?.innerHeight or document?.documentElement?.clientHeight or 0
  sw: screen?.width or 0
  sh: screen?.height or 0
  mobile: mobile
  mobileOffset: if mobile then 60 else 0
